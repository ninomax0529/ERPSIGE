 ...
 */
package controlador.cxp.avanceSuplidor;

import com.jfoenix.controls.JFXButton;
import com.jfoenix.controls.JFXCheckBox;
import com.jfoenix.controls.JFXDatePicker;
import com.jfoenix.controls.JFXTextField;
import controlador.cxp.comprobanteDePago.RegistroComprobanteDePagoController;
import java.io.File;
 ...
 */
public class AvanceASuplidorController implements Initializable {
...
//    TaskImprimirRecibo taskImprimirRecibo;
    ...
    @FXML
    private TableView<AvanceASuplidor> tbAvanve;
    @FXML
    private TableColumn<AvanceASuplidor, AvanceASuplidor> tbcNumero;
    @FXML
    private TableColumn<AvanceASuplidor, String> tbcFechaUltimoAvance;
    @FXML
    private TableColumn<AvanceASuplidor, Double> tbcTotalAvance;
    @FXML
    private TableColumn<AvanceASuplidor, Double> tbcTotalPendiente;
    @FXML
    private TableColumn<AvanceASuplidor, String> tbcCliente;
    @FXML
    private TableView<DetalleAvanceASuplidor> tbDetalleAvanve;
    @FXML
    private TableColumn<DetalleAvanceASuplidor, String> tbcFechaAvanve;
    @FXML
    private TableColumn<DetalleAvanceASuplidor, Double> tbcAvanve;
    @FXML
    private TableColumn<DetalleAvanceASuplidor, String> tbcRecibo;
    @FXML
    private TableColumn<DetalleAvanceASuplidor, DetalleAvanceASuplidor> tbcVerDocumento;

    ObservableList<AvanceASuplidor> listaAvance = FXCollections.observableArrayList();
    ObservableList<DetalleAvanceASuplidor> listaDetaAvance = FXCollections.observableArrayList();
    @FXML
    ...
    private void iniciazarFiltro() {
        ...
        FilteredList<AvanceASuplidor> filteredData = new FilteredList<>(tbAvanve.getItems(), p -> true);
        ...
        txtFiltro.textProperty().addListener((observable, oldValue, newValue) -> {
            ...
            filteredData.setPredicate(filtro -> {
                ...
                } else if (filtro.getSuplidor() != null && filtro.getSuplidor().getDescripcion().contains(lowerCaseFilter)) {
                    return true; // Filter matches last name.
                }
        ...
        SortedList<AvanceASuplidor> sortedData = new SortedList<>(filteredData);
    ...
    private void iniciarTablaRecibo() {
        ...
        tbcCliente.setCellValueFactory(new PropertyValueFactory<>("suplidor"));
    ...
    public void iniciarTablaDetalle() {
        ...
        tbcRecibo.setCellValueFactory(new PropertyValueFactory<>("codigo"));
        ...
        tbcRecibo.setCellValueFactory(
                cellData -> {
                    SimpleStringProperty property = new SimpleStringProperty();
                    if (cellData.getValue() != null) {
                        property.setValue(cellData.getValue().getComprobanteDePago().getNumero().toString());
                    }
                    return property;
                });
        ...
        tbcVerDocumento.setCellFactory((TableColumn<DetalleAvanceASuplidor, DetalleAvanceASuplidor> param) -> {

            TableCell<DetalleAvanceASuplidor, DetalleAvanceASuplidor> cellsc = new TableCell<DetalleAvanceASuplidor, DetalleAvanceASuplidor>() {
                @Override
                public void updateItem(DetalleAvanceASuplidor item, boolean empty) {
...
//                        imageFile = new File(getClass().getResource("/imagen/img_documento.jpg").toString());
                        ...
                        hbox.setOnMouseClicked((event) -> {
                            ...
                            RptPago.getInstancia().imprimir(item.getComprobanteDePago().getCodigo());
                        });
    ...
    private void tbReciboMouseClicked(MouseEvent event) {
        ...
        if (!(tbAvanve.getSelectionModel().getSelectedIndex() == -1)) {
            ...
            AvanceASuplidor avanve = tbAvanve.getSelectionModel().getSelectedItem();

            listaDetaAvance.addAll(ManejoAvanceASuplidor
                    .getInstancia()
    ...
    private void totalAvance() {
        ...
        Double total = 0.00;
        for (AvanceASuplidor det : listaAvance) {
    ...
    private void totalAvanceDetalle() {
        ...
        Double total = 0.00;
        for (DetalleAvanceASuplidor det : listaDetaAvance) {
    ...
    private void btnNuevoActionEvent(ActionEvent event) throws IOException {
        ...
        loader.setLocation(getClass().getResource("/vista/cxp/comprabanteDePago/RegistroComprobanteDePago.fxml"));

        loader.load();

        Parent root = loader.getRoot();

        RegistroComprobanteDePagoController controller = loader.getController();

        controller.getHbDetPago().setVisible(false);
        ...
        listaAvance.clear();
        listaAvance.addAll(ManejoAvanceASuplidor.getInstancia().getLista());
    }
...
//        }
    ...
    private void buscar() {
        ...
        listaAvance.addAll(ManejoAvanceASuplidor.getInstancia().getLista(fechaini, fechafin));
