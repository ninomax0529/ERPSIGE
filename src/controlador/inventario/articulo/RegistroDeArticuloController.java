 ...
 */
public class RegistroDeArticuloController implements Initializable {
    ...
    @FXML
    private JFXComboBox<?> cbUnidadEntrada1;
    @FXML
    private JFXTextField txtPrecioCompra1;
    @FXML
    private JFXButton btnCalcularPv1;
...
//                return;
            ...
            if (tbArticuloUnidad.getItems().size() <= 0) {
            ...
            Double precioCompra = Double.parseDouble(txtPrecioCompra.getText().isEmpty() ? "0" : txtPrecioCompra.getText()),
                    ...
                    precioVenta1 = Double.parseDouble("0"),
...
//                    margenUtilidad3 = Double.parseDouble(txtMargenUtilidad3.getText().isEmpty() ? "0" : txtMargenUtilidad3.getText());
            ...
            String codigoBarra = txtcodigoBarra.getText().isEmpty() ? "GFSC" + txtCodigo.getText() : txtcodigoBarra.getText();
            ...
            getArticulo().setNombre(txtNombre.getText().trim());
            ...
            ArticuloUnidad articuloUnidad = listaArticuloUnidad.get(0);

            for (ArticuloUnidad au : listaArticuloUnidad) {

                if (au.getCantidadUnidades() <= 0) {

                    ClaseUtil.mensaje(" No puede haber cantidad de unidades en cero en la configuracion de unidades y  costo  ");
                    return;
                }
                au.setUnidadInventario(false);

            }

            articuloUnidad.setUnidadInventario(true);

            String nombreRuta = " ";

            //Buscamos la unidad en que se guardara el inventario    
            Unidad uniInventario = articuloUnidad.getUnidad();
            Double factorVenta = articuloUnidad.getCantidadUnidades();

            for (ArticuloUnidad au : listaArticuloUnidad) {

                if (au.getCantidadUnidades() > articuloUnidad.getCantidadUnidades()) {

                    uniInventario = au.getUnidad();
                    au.setUnidadInventario(true);

                    factorVenta = au.getCantidadUnidades();
                    articuloUnidad = au;
...
//                getArticulo().setExistenciaArticuloCollection(listaExistenciaArticulo);
//                setArticulo(ManejoArticulo.getInstancia().actualizar(getArticulo()));
//
//                existenciaArticulo = ManejoExistenciaArticulo.getInstancia().getExistenciaArticulo(getArticulo().getCodigo(), 1);
//
//                existenciaArticulo.setUnidad(uniInventario);
//                existenciaArticulo.setNombreUnidadBase(uniInventario.getDescripcion());
//
//                ManejoExistenciaArticulo.getInstancia().actualizar(existenciaArticulo);
...
//
//                ext.setUnidad(uniInventario);
//                ext.setNombreUnidadBase(uniInventario.getDescripcion());
//                ext.setUnidadDeNegocio(getArticulo().getUnidadDeNegocio());
//
//                ManejoExistenciaArticulo.getInstancia().actualizar(ext);
//            }
            //Confugurando la unidad de venta en todas sus unidades
            for (ArticuloUnidad au : listaArticuloUnidad) {

                au.setFatorVenta(factorVenta / au.getCantidadUnidades());
                au.setUnidadEntrada(cbUnidadEntrada.getSelectionModel().getSelectedItem().getCodigo());
                au.setNombreUe(cbUnidadEntrada.getSelectionModel().getSelectedItem().getDescripcion());
                au.setArticulo(getArticulo());
                au.setFechaRegistro(new Date());
                System.out.println("unidad " + uniInventario);

                if (isEditar()) {
                    ManejoArticuloUnidad.getInstancia().actualizar(au);
                } else {
                    ManejoArticuloUnidad.getInstancia().salvar(au);
                }

            }

//            List<ArticuloUnidad> listaArtUnidad = null;
//            double factorUnidad = 1.0;
            for (ExistenciaArticulo ext : listaExistenciaArticulo) {

//                listaArtUnidad = ManejoArticuloUnidad.getInstancia()
//                        .getListaUnidadSslida(ext.getArticulo().getCodigo());
//
//                if (listaArtUnidad.size() == 3) {
//                    factorUnidad = listaArtUnidad.get(1).getFatorVenta();
//
//                } else if (listaArtUnidad.size() == 2) {
//
//                    factorUnidad = listaArtUnidad.get(1).getFatorVenta();
//                }
                ext.setUnidad(uniInventario);
                ext.setNombreUnidadBase(uniInventario.getDescripcion());
                ext.setUnidadDeNegocio(getArticulo().getUnidadDeNegocio());
//                ext.setExistencia(ext.getExistencia() * factorUnidad);
...
//
//                existenciaArticulo = new ExistenciaArticulo();
...
//        final FileChooser fileChooser = new FileChooser();
            ...
            try {
                ...
                Logger.getLogger(RegistroDeArticuloController.class.getName()).log(Level.SEVERE, null, ex);
            }
    ...
    private void agregarAlistaSDePrecioGeneral() {
        ...
        try {
            ...
            for (ArticuloUnidad articuloUnidad : listaArticuloUnidads) {
                ...
                detalleListaDePrecio.setPrecio(articuloUnidad.getPrecioVenta());

                detalleListaDePrecio.setHabilitado(true);
                detalleListaDePrecio.setCostoUnitario(articuloUnidad.getCostoUnitario());
                detalleListaDePrecio.setUnidadSalida(articuloUnidad.getUnidad());
                ...
                for (DetalleListaDePrecio dpl : lisDetalleListaDePrecios) {
                    ...
                    if (Objects.equals(dpl.getArticulo().getCodigo(), detalleListaDePrecio.getArticulo().getCodigo())
                            && Objects.equals(dpl.getUnidadSalida().getCodigo(), detalleListaDePrecio.getUnidadSalida().getCodigo())) {
    ...
    private void btnAgregarArticalmacenActionEvent(ActionEvent event) {
        ...
        try {
            ...
            if (!(tbAlmacen.getSelectionModel().getSelectedIndex() == -1)) {
                ...
                existenciaArticulo.setUnidad(listaArticuloUnidad.get(0).getUnidad());
