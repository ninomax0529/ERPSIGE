...
public class FormaDePagoController implements Initializable {
    ...
    @FXML
    private JFXTextField txtTranferencia;
    @FXML
...
//        tipoFormaPago = new TipoFormaPago();
    ...
    private void btnAplicarActionEvent(ActionEvent event) {
        ...
        Double transferencia = Double.parseDouble(txtTranferencia.getText().isEmpty() ? "0" : txtTranferencia.getText());
        Double deposito = Double.parseDouble(txtDeposito.getText().isEmpty() ? "0" : txtDeposito.getText());
        Double total = Double.parseDouble(txtTotalFactura.getText());

        getListaFormaPago().clear();
        ...
        if (transferencia > 0) {
            ...
            formaPago = new FormaPago();
            tipoFormaPago = ManejoTipoFormaPago.getInstancia().getTipoFormaPago(6);
            formaPago.setTipoFormaPago(tipoFormaPago);
            formaPago.setMonto(transferencia);
            getListaFormaPago().add(formaPago);
        ...
        if (deposito > 0) {
            ...
            formaPago = new FormaPago();
            tipoFormaPago = ManejoTipoFormaPago.getInstancia().getTipoFormaPago(5);
            formaPago.setTipoFormaPago(tipoFormaPago);
            formaPago.setMonto(deposito);
            getListaFormaPago().add(formaPago);

        }

        totalPago = efectivo + tarjeta + transferencia + cheque + deposito;
