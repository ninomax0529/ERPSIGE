 ...
 */
public class ManejoDatosDeVehiculo extends ManejoEstandar<DatosDeVehiculo> {

    private static ManejoDatosDeVehiculo manejoDatosDeVehiculo = null;
    private Session session = HibernateUtil.getSession();

    public static ManejoDatosDeVehiculo getInstancia() {
        if (manejoDatosDeVehiculo == null) {
            manejoDatosDeVehiculo = new ManejoDatosDeVehiculo();
        }
        return manejoDatosDeVehiculo;
    }
    ...
    public List<DatosDeVehiculo> getListaDatosVehiculo(int cliente) {

        List<DatosDeVehiculo> lista;
        String query = " SELECT dv.* from  contrato_de_servicio ct INNER JOIN detalle_contrato_de_servicio dct "
                + " on ct.codigo=dct.contrato_de_servicio  \n"               
                + " INNER JOIN  datos_de_vehiculo dv on dv.servicio=dct.codigo \n"
                + "  where  ct.cliente=:cliente ";

        lista = session.createSQLQuery(query)
                .addEntity(DatosDeVehiculo.class).setParameter("cliente", cliente).list();
...
//                + " ";
    ...
    public static void main(String[] args) {
        ...
        List<DatosDeVehiculo> lista=getInstancia().getListaDatosVehiculo(77);
        for (DatosDeVehiculo arg : lista) {
            System.out.println("datos de vehiculo : "+arg.getVehiculo());
